...
from utils import HTTPClient, extract_pdf_robust, is_poor_text
...
class SourcePriority:
    ...
    @staticmethod
    def should_extract_pdf(url: str, title: str, size_mb: float) -> bool:
...
def extract_pdf_text(url: str, title: str, http_client: HTTPClient) -> Optional[str]:
    try:
        pdf_bytes, _ = http_client.get_bytes(url)
        size_mb = len(pdf_bytes) / (1024 * 1024)
        
        if not SourcePriority.should_extract_pdf(url, title, size_mb):
            logger.debug(f"      Skip: {size_mb:.1f}MB")
            return None

...
=====================================================
Version 5.1 - Corrigé (faute de frappe __future__)

- Cible l'API JSON '/PAE/Portal/Recherche/Search.svc/Search'.
- Gère sa propre instance 'httpx.Client' pour les requêtes POST.
- Les paramètres (région, année) sont fournis par CLI.
- Ajout d'une sortie CSV.

ATTENTION : La logique de construction du payload (_build_query_payload)
et de parsing (_build_project_from_side_v2) est BASÉE SUR L'ANALYSE
FAITE POUR L'OCCITANIE (V4). Elle échouera probablement pour d'autres
régions sans une adaptation manuelle de ces fonctions.
"""

from __future__ import annotations  # <-- LIGNE CORRIGÉE
...
# ======================================================================
# Écriture CSV (Nouvelle fonctionnalité)
# ======================================================================
...
def main():
    parser = argparse.ArgumentParser(description="Scraper générique pour le portail SIDE des DREAL.")
    
    parser.add_argument(
        "-l", "--region-label", 
        type=str, 
        required=True, 
        help="Le nom complet de la région (pour le filtre). Ex: 'Occitanie'"
    )
    parser.add_argument(
        "-s", "--region-slug", 
        type=str, 
        required=True, 
        help="Le slug de la région (pour la sortie). Ex: 'occitanie'"
    )
    parser.add_argument(
        "-y", "--year", 
        type=str, 
    ...
    parser.add_argument(
        ...
        type=str, 
        required=False, 
        help="Filtre optionnel par département (NN). Ex: '31'"
    )
    parser.add_argument(
        "-o", "--output", 
        type=str, 
    ...
    if args.output:
        ...
        output_file = Path(f"projets_bess_{args.region_slug}_{args.year}.csv")
    ...
    _write_to_csv(projects, output_file)
